...
# ------------------ THEME ------------------
PRIMARY_BG = "#103C43"     # deep teal background
PANEL_BG   = "#0E353B"     # darker teal panel
ACCENT_YEL = "#F3C83E"     # yellow titles / buttons
ACCENT_RED = "#E64E43"     # red menu/op buttons
TEXT_WHITE = "#EAF7FA"
...
<style>
  ...
  .row {{ display:grid; grid-template-columns: 1.2fr 1.2fr .6fr; gap:10px; align-items:center; margin-bottom:8px; }}
  ...
  /* Hydraulic grid */
  .hydro-grid {{ display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:10px; }}
  .hydro-btn {{ background:{ACCENT_YEL}; color:#111; font-weight:900; border:none; border-radius:10px; padding:18px 12px; text-align:center; }}
</style>
...
with left:
    ...
    # Header row (PROCESS NAME | SETTING | NOTE)
...
st.caption("Pixel-style clone of the DABO 'Mold Setting' screen. Set as main file: mold_setting_clone.py")
